...
from database.base_model import Register, Login, RePassword, GoogleToken, Question, Admin, Report, CreateTemplate, CreateSection, UpdateTemplate, SelectMainTemplate, SelectSeconderyTemplate
...
async def create_history(section_id: str, question: Question, access_token: str = Cookie(None)):
    ...
    history_template_info = history_template_collection.find_one({"_id": ObjectId(res_info[0]["select_main"])})
        
    llm = main(
        model_name=MODEL_NAME,
        system_template=history_template_info.get("template"),
        text=question.human,
...
async def chatbot_secondery(section_id: str, question: Question, access_token: str = Cookie(None)):
    ...
    history_template_info = history_template_collection.find_one({"_id": ObjectId(res_info[0]["select_secondery"])})
...
@app.get("/history-template/{section_id}")
async def history_template(section_id: str, access_token: str = Cookie(None)):
    payload = decode_access_token(token=access_token)
    ...
    result = loop_data(res)
    
    return result

# API for Update History Template
@app.put("/history-template/{_id}")
async def update_template(_id: str, form_data: UpdateTemplate, access_token: str = Cookie(None)):
    payload = decode_access_token(token=access_token)
...
@app.post("/history-template/{section_id}")
async def create_template(section_id: str, form_data: CreateTemplate, access_token: str = Cookie(None)):
    payload = decode_access_token(token=access_token)
...
@app.delete("/history-template/{_id}")
async def delete_template(_id: str, access_token: str = Cookie(None)):
    payload = decode_access_token(token=access_token)
    ...
    if ((res_info[0]["select_main"] == _id) | (res_info[0]["select_secondery"] == _id)):
...
@app.get("/section-template")
async def section_template(access_token: str = Cookie(None)):
    payload = decode_access_token(token=access_token)
...
@app.post("/section-template")
async def create_section_template(form_data: CreateSection, access_token: str = Cookie(None)):
    payload = decode_access_token(token=access_token)
...
@app.delete("/section-template/{section_id}")
async def delete_section_template(section_id: str, access_token: str = Cookie(None)):
    payload = decode_access_token(token=access_token)
    ...
    sec_info = history_template_collection.find({"section_id": section_id})
    result = loop_data(sec_info)
    
    target_id = [res_info[0]["select_main"], res_info[0]["select_secondery"]]
    exits_ids = {item["_id"] for item in result}
    found = any(target in exits_ids for target in target_id)
    if (found):
        raise HTTPException(status_code=401, detail="not allowed delete")
...
@app.put("/select-main-template/{user_id}")
async def select_main_template(user_id: str, form_data: SelectMainTemplate, access_token: str = Cookie(None)):
    payload = decode_access_token(token=access_token)
...
@app.put("/select-secondry-template/{user_id}")
async def select_main_template(user_id: str, form_data: SelectSeconderyTemplate, access_token: str = Cookie(None)):
    payload = decode_access_token(token=access_token)
