...
async def get_user(
    ...
    return {"user_id": user_id, "section_id": section_id, "role": role}
...
async def google_login(google_token: GoogleToken):
    ...
    token = create_access_token(
        data={"sub": email}, minutes=ACCESS_TOKEN_EXPIRE_MINUTES
    )
    ...
    user_collection.insert_one(
        {"username": email_sp[0], "email": email, "role": "user"}
    )
...
async def send_email(email: str):
    ...
    retult = _send_email(
        title="Verify Email", from_=smpt_user, to_=email, content=message
    )
    verify_token = create_access_token(
        data={"sub": email}, minutes=VERIFY_TOKEN_EXPIRE_MINUTES
    )
...
async def register(form_data: Register, verify_tk: str, verification_code: str):
    ...
    if playload == "Invalid token":
        return "Invalid token"
    if playload == "Token expired":
        ...
        if form_data.verify_code == verification_code:
            ...
            token = create_access_token(
                data={"sub": form_data.username}, minutes=ACCESS_TOKEN_EXPIRE_MINUTES
            )
            responese = JSONResponse(content={"msg": "Register successful"})
            responese.set_cookie(
                key="access_token",
...
async def forgot_password(
    form_data: RePassword, verify_tk: str, verification_code: str
):
    playload = decode_access_token(token=verify_tk)
    if playload == "Invalid token":
        return "Invalid token"
    if playload == "Token expired":
        ...
        if form_data.verify_code == verification_code:
            ...
            user_collection.update_one(
                {"email": form_data.email},
                {"$set": {"password": hash_password.decode("utf-8")}},
            )

            return {"msg": "Update password success"}
        else:
            return "verification_code fail"
...
async def get_section(user_id: str, access_token: str = Cookie(None)):
    ...
    res = section_collection.find({"user_id": user_id})
...
async def delete_section(section_id: str, access_token: str = Cookie(None)):
    ...
    return {"msg": "delete section successful"}
...
async def delete_section(section_id: str):
    ...
    return {"msg": "delete section non-user successful"}


# API for Create History (Main Chatbot)
@app.post("/main-chatbot/{section_id}")
async def create_history(
    section_id: str, question: Question, access_token: str = Cookie(None)
):
    payload = decode_access_token(token=access_token)
    ...
    admin_collection.update_one(
        {"_id": ObjectId(res_info[0]["_id"])}, {"$set": update_tokens}
    )
...
async def chatbot_secondery(
    ...
    return {"question": question.human, "answer": llm["response"]}
...
async def get_chatbot_config(user_id: str, access_token: str = Cookie(None)):
    ...
    return {
        ...
        "prompt_template_just_venting": chatbot_config.get(
            "prompt_template_just_venting"
        ),
        "prompt_template_test": chatbot_config.get("prompt_template_test"),
        "prompt_template_just_venting_test": chatbot_config.get(
...
async def create_report(report: Report):
    ...
    return {"msg": "create report successful"}
...
async def delete_report(user_id: str, report_id: str, access_token: str = Cookie(None)):
    ...
    return {"msg": "delete report successful"}


# API for Test Chatbot (Admin)
@app.post("/test-main-chatbot/{user_id}/{section_id}")
async def chatbot_admin(
    user_id: str, section_id: str, form_data: Admin, access_token: str = Cookie(None)
):
    payload = decode_access_token(token=access_token)
...
async def get_count_all_user(user_id: str, access_token: str = Cookie(None)):
    ...
    return {"msg": "get user count successful", "total_user": user_count}


# API for Update Prompt Template
@app.put("/update-prompt-template/{user_id}")
async def update_prompt_template(
    user_id: str, form_data: PromptUpdate, access_token: str = Cookie(None)
):
    payload = decode_access_token(token=access_token)
    ...
    update_prompt = {"prompt_template": form_data.prompt_template}

    admin_collection.update_one({"user_id": user_id}, {"$set": update_prompt})

    return {"msg": "update prompt template successful"}


# API for Update prompt_justventing
@app.put("/update-prompt-justventing/{user_id}")
async def update_prompt_templateJV(
    user_id: str, form_data: PromptUpdateJV, access_token: str = Cookie(None)
):
    payload = decode_access_token(token=access_token)
...
async def prompt_template_test(
    user_id: str, form_data: PromptUpdateTS, access_token: str = Cookie(None)
):
    payload = decode_access_token(token=access_token)
    ...
    update_prompt = {"prompt_template_test": form_data.prompt_template_test}
...
async def update_prompt_templateJVTS(
    user_id: str, form_data: PromptUpdateJVTS, access_token: str = Cookie(None)
):
    payload = decode_access_token(token=access_token)
